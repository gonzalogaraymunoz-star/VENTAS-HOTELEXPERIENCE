    ...
    <main className="workspace">
      <header className="topbar">
        ...
        <div className="top-actions">
          ...
          <button className="button dark" onClick={()=>startQuote()}><Plus size={16}/> Cotizar</button>
        </div>
      </header>
...
function NewSale({ data, profile, initialProductId, onCreated }:{data:AppData;profile:Profile;initialProductId?:string;onCreated:()=>Promise<void>}) {
  ...
  const [channel,setChannel]=useState('Recepción'); const [priority,setPriority]=useState('Media'); const [checkin,setCheckin]=useState(''); const [checkout,setCheckout]=useState(''); const [contact,setContact]=useState(''); const [sellerProfileId,setSellerProfileId]=useState(profile.id); const [notes,setNotes]=useState('');
  const [passengers,setPassengers]=useState<PassengerDraft[]>([blankPassenger(true)]);
  const [services,setServices]=useState<ServiceDraft[]>([]); const [busy,setBusy]=useState(false); const [message,setMessage]=useState(''); const [createdCode,setCreatedCode]=useState(''); const [createdLeadId,setCreatedLeadId]=useState(''); const [createdStatus,setCreatedStatus]=useState<'quoted'|'confirmed'|''>('');
  const [catalogOpen,setCatalogOpen]=useState(false); const [partnerOpen,setPartnerOpen]=useState(false); const [partnerBusy,setPartnerBusy]=useState(false); const [partnerMessage,setPartnerMessage]=useState('');
  ...
  function addProductById(id:string){const p=data.products.find(x=>x.id===id);if(!p)return;const pax=Math.max(1,passengers.length);const known=resolveProductPrice(p,pax);setServices(prev=>[...prev,{product_id:p.id,product_code:p.code,product_name:p.name,category:p.category,date:'',start_time:'',pax,modality:p.price_mode,unit_price:known??0,operator_cost:0,supplier_id:'',supplier_name:'',hotel_commission_pct:15,seller_commission_pct:5,notes:''}])}
  ...
  return <div className="screen-stack sale-builder"><section className="page-heading sticky-title"><div><p className="eyebrow">COTIZACIÓN / VENTA</p><h1>Una oportunidad, un código, dos momentos.</h1><p className="muted">Primero puedes guardar el trabajo comercial. Solo Confirmar venta habilita Operaciones, pagos y comisiones.</p></div>{topActions}</section>
    ...
    <section className="builder-section"><SectionNumber n="01" title="Origen y responsable" text="El negocio y el vendedor quedan vinculados a la venta."/><div className="form-grid four"><label>Hotel / negocio<div className="inline-field-action"><select value={hotelId} onChange={e=>setHotelId(e.target.value)}><option value="">Seleccionar…</option>{data.hotels.map(h=><option key={h.id} value={h.id}>{h.name} · {h.lead_prefix||'sin prefijo'}</option>)}</select><button type="button" className="button ghost" onClick={()=>setPartnerOpen(true)}><Building2 size={15}/> Ingresar</button></div></label><label>Canal<select value={channel} onChange={e=>setChannel(e.target.value)}>{channels.map(c=><option key={c}>{c}</option>)}</select></label><label>Vendedor<select value={sellerProfileId} onChange={e=>setSellerProfileId(e.target.value)}><option value="">Seleccionar persona…</option>{availableSellers.map(s=><option key={s.id} value={s.id}>{s.full_name||s.email} · {s.role}</option>)}</select></label><label>Prioridad<select value={priority} onChange={e=>setPriority(e.target.value)}><option>Alta</option><option>Media</option><option>Baja</option></select></label></div><div className="form-grid three"><label>Contacto principal<input value={contact} onChange={e=>setContact(e.target.value)} placeholder="Teléfono, email o ambos"/></label><label>Check-in<input type="date" value={checkin} onChange={e=>setCheckin(e.target.value)}/></label><label>Check-out<input type="date" value={checkout} onChange={e=>setCheckout(e.target.value)}/></label></div></section>
    ...
    {catalogOpen&&<div className="quote-overlay"><div className="quote-overlay-panel"><header className="quote-overlay-head"><div><p className="eyebrow">SELECCIÓN DE PRODUCTOS</p><strong>{services.length} agregado(s) a esta cotización</strong></div><button className="button dark" onClick={()=>setCatalogOpen(false)}>Volver a cotización <X size={16}/></button></header><div className="quote-overlay-body"><ProductWorkspace products={data.products} onQuote={addProductById}/></div><footer className="quote-overlay-footer"><span>{services.length} producto(s) seleccionados</span><button className="button dark big" onClick={()=>setCatalogOpen(false)}>Continuar con la cotización <ArrowRight size={16}/></button></footer></div></div>}
