...
import { addPayment, createSale, loadLeads, loadPayments, loadReferenceData, loadServices, updateLeadStatus } from '../lib/sales';
...
const pipelineStages = ['nuevo', 'contactado', 'cotizando', 'propuesta', 'confirmado', 'perdido'];
...
export default function SalesApp({ profile }: { profile: Profile }) {
  ...
  const nav: { id: Screen; label: string; icon: any }[] = [
    ...
    { id: 'new-sale', label: 'Nueva venta', icon: ShoppingBag },
    ...
    <main className="workspace">
      <header className="topbar">
        ...
        <div className="top-actions">
          ...
          <button className="button dark" onClick={()=>go('new-sale')}><Plus size={16}/> Nueva venta</button>
        </div>
      </header>
      ...
      <section className="content-area">
        ...
        {loading && data.leads.length===0 ? <div className="loading-panel">Cargando información comercial…</div> : <>
          ...
          {screen==='leads' && <LeadTable leads={data.leads} services={data.services}/>} 
          ...
          {screen==='payments' && <Payments payments={data.payments} services={data.services} onAdded={refresh}/>} 
        </>}
      </section>
    </main>
  </div>;
...
function titleFor(screen: Screen) {
  return ({ dashboard:'Centro comercial', 'new-sale':'Crear venta', leads:'Clientes y ventas', pipeline:'Pipeline comercial', products:'Catálogo', payments:'Pagos' } as Record<Screen,string>)[screen];
}

function Dashboard({ data, onNavigate }: { data: AppData; onNavigate: (screen: Screen)=>void }) {
  const confirmed = data.leads.filter(l=>l.estado==='confirmado' || l.commercial_status==='won');
  const sales = data.services.reduce((sum,s)=>sum+Number(s.precio_total ?? s.precio_venta ?? 0),0);
  const margin = data.services.reduce((sum,s)=>sum+Number(s.margen_hotel_experience ?? 0),0);
  const pendingOps = data.services.filter(s=>s.estado_operacion==='Pendiente').length;
  const pendingPayment = data.services.filter(s=>s.estado_pago!=='Pagado').length;
  const recent = data.leads.slice(0,8);
  return <div className="screen-stack">
    <section className="hero-row">
      <div><p className="eyebrow">HOY</p><h1>Vende. Registra. Entrega a operación.</h1><p className="muted large">Una sola ficha conecta huésped, acompañantes, productos, pagos y margen.</p></div>
      <button className="button dark hero-cta" onClick={()=>onNavigate('new-sale')}>Crear venta <ArrowRight size={17}/></button>
    </section>
    <section className="metric-grid">
      <Metric label="Clientes" value={String(data.leads.length)} hint={`${confirmed.length} confirmados`} />
      <Metric label="Venta registrada" value={clp(sales)} hint={`${data.services.length} productos`} />
      <Metric label="Margen LINK/HE" value={clp(margin)} hint="después de costos y comisiones" />
    ...
    <section className="two-column-grid">
      ...
      <div className="panel">
        <PanelHead title="Handoff a Operaciones" action="Pipeline" onClick={()=>onNavigate('pipeline')}/>
        <div className="handoff-flow"><FlowStep n="1" title="Venta confirmada" text="Lead y reserva comparten código."/><FlowStep n="2" title="Productos vendidos" text="Cada servicio recibe su código y economía."/><FlowStep n="3" title="Operación pendiente" text="HOTEL EXPERIENCE toma los servicios confirmados."/></div>
      </div>
    </section>
  </div>;
}
...
function LeadTable({ leads, services }: { leads: Lead[]; services: LeadService[] }) {
  ...
  const serviceCount=(leadId:string)=>services.filter(s=>s.lead_id===leadId).length;
  return <div className="screen-stack">
    <section className="page-heading"><div><p className="eyebrow">CRM COMERCIAL</p><h1>Clientes</h1><p className="muted">El código del lead acompaña toda la vida de la venta.</p></div></section>
    ...
    <div className="table-shell"><table><thead><tr><th>Código</th><th>Origen</th><th>Contacto</th><th>Productos</th><th>Total</th><th>Estado</th><th>Creado</th></tr></thead><tbody>{rows.map(l=><tr key={l.id}><td><strong>{l.codigo}</strong><small>{l.reserva}</small></td><td>{l.hotel_partners?.name || l.canal || '—'}</td><td>{l.contacto || '—'}</td><td>{serviceCount(l.id)}<small>{l.servicio || 'Sin detalle'}</small></td><td>{clp(l.precio_venta)}</td><td><Status value={l.estado}/></td><td>{new Date(l.created_at).toLocaleDateString('es-CL')}</td></tr>)}</tbody></table>{!rows.length&&<Empty text="No hay coincidencias."/>}</div>
  </div>;
}

function Pipeline({ leads, onChange }: { leads: Lead[]; onChange:(id:string,status:string)=>Promise<void> }) {
  const [busy,setBusy]=useState('');
  return <div className="screen-stack"><section className="page-heading"><div><p className="eyebrow">CONVERSIÓN</p><h1>Pipeline</h1><p className="muted">La venta termina en Confirmado; desde ahí la operación toma el relevo.</p></div></section><div className="kanban-board">{pipelineStages.map(stage=><section className="kanban-column" key={stage}><header><strong>{stage}</strong><span>{leads.filter(l=>(l.estado||'nuevo').toLowerCase()===stage).length}</span></header><div>{leads.filter(l=>(l.estado||'nuevo').toLowerCase()===stage).map(l=><article className="kanban-card" key={l.id}><div className="kanban-code">{l.codigo}</div><strong>{l.servicio || 'Lead sin producto'}</strong><p>{l.hotel_partners?.name || l.canal || 'Sin origen'}</p><small>{clp(l.precio_venta)}</small><select disabled={busy===l.id} value={(l.estado||'nuevo').toLowerCase()} onChange={async e=>{setBusy(l.id);try{await onChange(l.id,e.target.value)}finally{setBusy('')}}}>{pipelineStages.map(s=><option key={s}>{s}</option>)}</select></article>)}</div></section>)}</div></div>;
}
...
function Products({ products }: { products: Product[] }) {
  ...
  return <div className="screen-stack"><section className="page-heading"><div><p className="eyebrow">SUPABASE · FUENTE DE VERDAD</p><h1>Productos</h1><p className="muted">La app comercial no mantiene un catálogo paralelo.</p></div></section><div className="toolbar wrap"><div className="search-box"><Search size={17}/><input placeholder="Buscar producto…" value={query} onChange={e=>setQuery(e.target.value)}/></div><select value={category} onChange={e=>setCategory(e.target.value)}>{categories.map(c=><option key={c}>{c}</option>)}</select></div><div className="product-grid">{rows.map(p=><article className="product-card" key={p.id}><span className="product-code">{p.code}</span><h3>{p.name}</h3><p>{p.description || p.category}</p><div><span>{p.category}</span><strong>{priceLabel(p)}</strong></div></article>)}</div></div>;
}
function priceLabel(p:Product){const v=resolveProductPrice(p,2);return v===null?'Cotización':`${clp(v)}${Object.prototype.hasOwnProperty.call(p.prices,'2')?' p/p':''}`}
...
function NewSale({ data, profile, onCreated }:{data:AppData;profile:Profile;onCreated:()=>Promise<void>}) {
  ...
  const [services,setServices]=useState<ServiceDraft[]>([]); const [productId,setProductId]=useState(''); const [busy,setBusy]=useState(false); const [message,setMessage]=useState(''); const [createdCode,setCreatedCode]=useState('');
