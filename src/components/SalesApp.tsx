...
import { confirmSale, createSale, loadLeads, loadPayments, loadReferenceData, loadServices, updateLeadStatus } from '../lib/sales';
...
function NewSale({ data, profile, initialProductId, onCreated }:{data:AppData;profile:Profile;initialProductId?:string;onCreated:()=>Promise<void>}) {
  ...
  const [services,setServices]=useState<ServiceDraft[]>([]); const [productId,setProductId]=useState(initialProductId||''); const [busy,setBusy]=useState(false); const [message,setMessage]=useState(''); const [createdCode,setCreatedCode]=useState(''); const [createdLeadId,setCreatedLeadId]=useState(''); const [createdStatus,setCreatedStatus]=useState<'quoted'|'confirmed'|''>('');

  useEffect(()=>{if(!hotelId&&data.hotels[0])setHotelId(data.hotels[0].id)},[data.hotels,hotelId]);
  useEffect(()=>{setProductId(initialProductId||'')},[initialProductId]);
