...
export function ActionDashboard({
  ...
  return <div className="screen-stack action-dashboard">
    <section className="work-hero">
      <div><p className="eyebrow">CENTRO DE TRABAJO</p><h1>¿Qué hay que hacer ahora?</h1><p>Inicio muestra solo trabajo accionable: ingresos/ventas por cerrar, dinero por cobrar y ventas que deben pasar a ejecución.</p></div>
      <button className="button dark big" onClick={onQuote}><ShoppingBag size={17}/> Nuevo ingreso / venta</button>
    </section>

    <section className="work-summary-grid">
      <button onClick={onClients} className="work-summary-card"><span><FileText size={17}/> Ingresos / ventas por cerrar</span><strong>{quoteRows.length}</strong><small>{clp(quoteTotal)} en oportunidades abiertas</small><ChevronRight size={17}/></button>
    ...
    <section className="work-columns">
      <WorkPanel title="Ingresos / ventas por cerrar" eyebrow="VENTAS" action="Ver clientes" onAction={onClients}>
        ...
        {!quoteRows.length && <WorkEmpty title="Sin ingresos o ventas esperando cierre." text="Puedes crear una nueva oportunidad desde Ingreso / venta."/>}
      </WorkPanel>
  ...
  return <div className="screen-stack catalog-workspace">
    ...
    <section className="catalog-browser-layout">
      ...
      {selected&&primary&&<div className="catalog-detail">
        ...
        <div className="pax-control"><div><Users size={18}/><span><strong>Calcular para</strong><small>La tarifa cambia según pax cuando el catálogo tiene tramos.</small></span></div><div className="pax-stepper-v2"><button onClick={()=>setPax(Math.max(1,pax-1))}><Minus size={15}/></button><strong>{pax}</strong><span>pax</span><button onClick={()=>setPax(Math.min(12,pax+1))}><Plus size={15}/></button></div></div>
        <div className="variant-grid">{selected.products.map(product=>{const quote=catalogQuote(product,pax);return <article key={product.id} className={quote.valid?'variant-card':'variant-card manual'}><div className="variant-top"><span><strong>{priceModeLabel(product.price_mode)}</strong><small>{product.code}</small></span><i>{productTax(product)}</i></div>{quote.valid?<><div className="variant-price"><strong>{clp(quote.unitPrice)}</strong><span>{quote.unit}</span></div><div className="variant-total"><span>Total {pax} pax</span><strong>{clp(quote.total)}</strong></div></>:<div className="variant-manual"><strong>Precio manual</strong><span>No existe tarifa automática para este tramo.</span></div>}<button className="button dark wide" onClick={()=>onQuote(product.id)}><ShoppingBag size={15}/> Usar en ingreso / venta</button></article>})}</div>
        <div className="price-matrix-card"><header><div><Boxes size={17}/><span><strong>Tramos completos</strong><small>Selecciona una fila para cambiar el pax.</small></span></div><b>1–12 PAX</b></header><div className="price-matrix-scroll"><table><thead><tr><th>Pax</th>{selected.products.map(product=><th key={product.id}>{priceModeLabel(product.price_mode)}<small>{product.code}</small></th>)}</tr></thead><tbody>{PAX_STEPS.map(amount=><tr key={amount} className={amount===pax?'selected':''} onClick={()=>setPax(amount)}><td><strong>{amount}</strong></td>{selected.products.map(product=>{const quote=catalogQuote(product,amount);return <td key={product.id}>{quote.valid?<><strong>{clp(quote.unitPrice)}</strong><small>Total {clp(quote.total)}</small></>:<span className="matrix-manual">Manual</span>}</td>})}</tr>)}</tbody></table></div></div>
      </div>}
    </section>
  </div>;
}
